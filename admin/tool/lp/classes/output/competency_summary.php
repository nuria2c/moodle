...
class competency_summary implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $exporterframework = new competency_framework_exporter($this->framework, array('context' => $this->framework->get_context()));
        $this->data->framework = $exporterframework->export($output);

        $exportercomp = new competency_exporter($this->competency, array('context' => $this->framework->get_context()));
        $competency = $exportercomp->export($output);
        ...
        if ($this->relatedcompetencies) {
            foreach ($this->relatedcompetencies as $competency) {
                $exportercomp = new competency_exporter($competency, array('context' => $this->framework->get_context()));
                $this->data->relatedcompetencies[] = $exportercomp->export($output);
            }
        }
