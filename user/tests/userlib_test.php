...
class core_userliblib_testcase extends advanced_testcase {
    ...
    public function test_user_get_user_details() {
        ...
        role_assign($studentrole->id, $teacher->id, $coursecontext->id);
        ...
        // Get student details with required and excluded fields.
        $result = user_get_user_details($student, $course1, array(), array('profileimageurl', 'enrolledcourses'));
        $this->assertEquals($student->id, $result['id']);
        $this->assertEquals($studentfullname, $result['fullname']);
        $this->assertArrayNotHasKey('profileimageurl', $result);
        $this->assertArrayNotHasKey('enrolledcourses', $result);

        // Get student details with excluded fields.
        $result = user_get_user_details($student, $course1, array('id', 'fullname', 'profileimageurl'),
                array('profileimageurl', 'enrolledcourses'));
        $this->assertCount(2, $result);
        $this->assertEquals($student->id, $result['id']);
        $this->assertEquals($studentfullname, $result['fullname']);

        // Get exception for invalid required and excluded fields.
        $this->setExpectedException('moodle_exception');
        $result = user_get_user_details($student, $course1, array('wrongrequiredfield'));
        $result = user_get_user_details($student, $course1, array(), array('wrongexcludedfield'));
    }
}
