...
abstract class backup_cron_automated_helper {
    ...
    public static function remove_excess_backups($course, $now = null) {
        $config = get_config('backup');
        $totalkeep = (int)$config->backup_auto_keep;
        $storage = $config->backup_auto_storage;
        $removedays = (int)$config->backup_auto_remove_days;

        if ($totalkeep == 0 && $removedays == 0) {
        ...
        $removedcoursebackups = false;
        if ($storage == self::STORAGE_COURSE || $storage == self::STORAGE_COURSE_AND_DIRECTORY) {
            $removedcoursebackups = self::remove_excess_backups_from_course($course, $now);
        }

        // Clean up excess backups in the specified external directory.
        $removeddirectorybackups = false;
        if ($storage == self::STORAGE_DIRECTORY || $storage == self::STORAGE_COURSE_AND_DIRECTORY) {
            $removeddirectorybackups = self::remove_excess_backups_from_directory($course, $now);
        }

        if ($removedcoursebackups || $removeddirectorybackups) {
            return true;
        } else {
            return false;
        }
    ...
    protected static function remove_excess_backups_from_course($course, $now) {
        ...
        $backupstoremove = self::get_backups_to_remove($backupfiles, $now);
        if ($backupstoremove) {
            foreach ($backupstoremove as $backuptoremove) {
                $backuptoremove->delete();
            }
            mtrace('Removed ' . count($backupstoremove) . ' old backup file(s) from the automated filearea');
            return true;
        } else {
            return false;
        }
    ...
    protected static function remove_excess_backups_from_directory($course, $now) {
        ...
        if ($backupstoremove) {
            foreach ($backupstoremove as $backuptoremove) {
                unlink($dir . '/' . $backuptoremove);
            }
            mtrace('Removed ' . count($backupstoremove) . ' old backup file(s) from external directory');
            return true;
        } else {
            return false;
        }
    ...
    /**
     * Get the list of backup files to remove depending on the automated backup settings.
     *
     * @param array $backupfiles Existing backup files
     * @param int $now Starting time of the process
     * @return array Backup files to remove
     */
    public static function get_backups_to_remove($backupfiles, $now) {
        $config = get_config('backup');
        $totalkeep = (int)$config->backup_auto_keep;
        $removedays = (int)$config->backup_auto_remove_days;
        $numneverremove = (int)$config->backup_auto_num_never_remove;

        krsort($backupfiles);
        $tokeep = $totalkeep;
        if ($removedays > 0) {
            $removedayssecs = $removedays * DAYSECS;
            $tokeep = 0;
            $backupfileskeys = array_keys($backupfiles);
            foreach ($backupfileskeys as $timemodified) {
                $mustremovebackup = $timemodified < ($now - $removedayssecs);
                if ($mustremovebackup || $tokeep >= $totalkeep) {
                    break;
                }
                $tokeep++;
            }

            if ($tokeep < $numneverremove) {
                $tokeep = $numneverremove;
            }
        }
        ...
        if (count($backupfiles) <= $tokeep) {
            ...
            $backupstoremove = array_splice($backupfiles, $tokeep);
